...
#ifdef TFT_CS
  gpio_set_level(TFT_CS, 1); // Chip select high (inactive)
  gpio_set_direction(TFT_CS, GPIO_MODE_OUTPUT);
#endif

// Configure chip select for touchscreen controller if present
#ifdef TOUCH_CS
  gpio_set_level(TOUCH_CS, 1); // Chip select high (inactive)
  gpio_set_direction(TOUCH_CS, GPIO_MODE_OUTPUT);
#endif

#ifdef TFT_WR
  gpio_set_level(TFT_WR, 1); // Set write strobe high (inactive)
  gpio_set_direction(TFT_WR, GPIO_MODE_OUTPUT);
#endif

#ifdef TFT_DC
  gpio_set_level(TFT_DC, 1); // Data/Command high = data mode
  gpio_set_direction(TFT_DC, GPIO_MODE_OUTPUT);
#endif

#ifdef TFT_RST
  if (TFT_RST >= 0) {
    gpio_set_level(TFT_RST, 1); // Set high, do not share pin with another SPI device
  ...
  busDir(dir_mask, GPIO_MODE_OUTPUT);
...
#if defined(ESP32_PARALLEL)
    gpio_set_level(TFT_CS, 0); // Chip select low permanently
    gpio_set_direction(TFT_CS, GPIO_MODE_OUTPUT);
#else
  #ifdef TFT_CS
...
uint8_t TFT_eSPI::readcommand8(uint8_t cmd_function, uint8_t index)
{
  ...
  busDir(dir_mask, GPIO_MODE_INPUT);
...
***************************************************************************************/
void busDir(uint32_t mask, gpio_mode_t mode)
{
#ifdef ESP32_PARALLEL
...
int16_t TFT_eSPI::textWidth(const string& string)
{
  ...
  char buffer[len];
  strncpy(buffer, string.c_str(), len);  
